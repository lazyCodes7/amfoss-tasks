...
const showData = () => {
    fetchUsers(inputValue.value).then((res) => {
        console.log(res);
        unContainer.innerHTML= `UserName : <span class="main__profile-username">${res.data.username}</span>`
        nameContainer.innerHTML= `Name : <span class="main__profile-name">${res.data.name}</span>`
        bioContainer.innerHTML= `Avatar/Identicon: <span class="main__profile-avatar"><img src="${res.data.avatar_url}"></span>`
        avatarcontainer.innerHTML= `Bio: <span class="main__profile-username">${res.data.bio}</span>`
    })
}
